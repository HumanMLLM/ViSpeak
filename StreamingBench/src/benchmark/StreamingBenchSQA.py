...
class StreamingBenchSQA(Benchmark):
    ...
    def eval(self, data, model, output_path):
        StreamingBenchSQAEval(data, model, output_path, self.video_root)

def StreamingBenchSQAInit(data):
    pass

def StreamingBenchSQAEval(data, MODEL, output_path, video_root):
    for video_data in tqdm.tqdm(data):  
        context = ""
        # start_time = subset["time"].split('-')[0].strip()
        ...
        for subset in video_data:  
            for question in subset["questions"]:
                ...
                response = get_model_response(MODEL, file, inp, max(timestamp - 60, 0), timestamp, timestamp + 1)
