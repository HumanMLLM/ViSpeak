...
class audioEncoderProcessor:
    ...
    def process(self, wav, start_time=None, end_time=None):
        if type(wav) is str:
            waveform, sample_rate = torchaudio.load(wav)
        elif type(wav) is torch.Tensor:
            waveform, sample_rate = wav, 16000
        else:
            raise NotImplementedError
        
        if start_time is not None:
            waveform = waveform[:, start_time*sample_rate:end_time*sample_rate]
