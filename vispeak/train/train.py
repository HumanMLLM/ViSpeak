...
from vispeak.train.vispeak_trainer import ViSpeakTrainer

from vispeak.util.data_utils import make_supervised_data_module, DataArguments
...
class ModelArguments:
    ...
    pretrain_combination_modules: Optional[str] = field(default=None)
    mm_projector_type: Optional[str] = field(default="mlp2x_gelu")
    use_new_seg_token: bool = field(default=False)
    combination: str = field(default="linear_combine") # ['linear_combine', 'add', 'adaptive_combine']
    use_informative_head: bool = field(default=False)
    pretrain_informative_head: Optional[str] = field(default=None)
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ["mm_projector", "vision_tower", "vision_resampler", "audio_encoder", "combination"]
...
def train():
    ...
    existing_informative_head = False
    if hasattr(model, 'informative_head'):
        informative_head = copy.deepcopy(model.informative_head)
        del model.informative_head
    ...
    model.config.pooling_size = data_args.pooling_size
    ...
    data_args.use_informative_head = model_args.use_informative_head
    training_args.use_informative_head = model_args.use_informative_head
    ...
    training_args.use_new_seg_token = model_args.use_new_seg_token
    training_args.combination = model_args.combination
    ...
    if model_args.combination == "linear_combine" or model_args.combination == "adaptive_combine":
        for p in model.get_model().combination.parameters():
            p.requires_grad = True
    model.initialize_vision_tokenizer(model_args, tokenizer=tokenizer)

    if model_args.use_informative_head:
        # model.requires_grad_(False)
        if existing_informative_head:
            model.informative_head = informative_head
        else:
            model.bulid_informative_head(model_args.pretrain_informative_head)
        for p in model.informative_head.parameters():
            p.requires_grad = True

    total_params = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in model.parameters())
    trainable_params = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in model.parameters() if p.requires_grad)
    trainable_params_name = [{name: param.numel()} for name, param in model.named_parameters() if param.requires_grad]
    rank0_print(f"Total parameters: ~{total_params/1e6:.2f} MB")
    rank0_print(f"Trainable parameters: ~{trainable_params/1e6:.2f} MB")
    rank0_print(trainable_params_name)
    ...
    trainer = ViSpeakTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
