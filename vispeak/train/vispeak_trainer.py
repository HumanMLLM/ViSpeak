...
class ViSpeakTrainer(Trainer):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_projector_lr is not None:
                projector_parameters = [
                    ...
                    if "mm_projector" in name or "audio_encoder.adpter" in name
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # print(model.model.audio_encoder.adpter.project.weight.requires_grad)
        # if getattr(self.args, "use_informative_head", False):
        ...
        #     # Only save Adapter
        #     keys_to_match = ["informative_head"]
        ...
        #         torch.save(weight_to_save, os.path.join(output_dir, f"informative_head_params.bin"))
        if getattr(self.args, "tune_mm_mlp_adapter", False) or getattr(self.args, "tune_audio_mlp_adapter", False):
            if getattr(self.args, "use_new_seg_token", False):
                from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR
                ...
                if self.args.local_rank == 0 or self.args.local_rank == -1:
                    self.model.config.save_pretrained(output_dir)
                    torch.save(weight_to_save, os.path.join(output_dir, f"other_trainable_params.bin"))
            if getattr(self.args, "combination", "linear_combine") == "linear_combine" or getattr(self.args, "combination", "linear_combine") == "adaptive_combine":
                from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR
                ...
                if self.args.local_rank == 0 or self.args.local_rank == -1:
                    self.model.config.save_pretrained(output_dir)
                    torch.save(weight_to_save, os.path.join(output_dir, f"combination_params.bin"))
            ...
            super(ViSpeakTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or getattr(self.args, "tune_audio_mlp_adapter", False):
            pass
        else:
            super(ViSpeakTrainer, self)._save(output_dir, state_dict)
